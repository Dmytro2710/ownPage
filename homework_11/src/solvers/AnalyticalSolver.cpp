...
AnalyticalSolver::AnalyticalSolver(const dlink::DroneCfg& currentDrone, const dlink::AmmoCfg& currentAmmo)
    : currentDrone(currentDrone), currentAmmo(currentAmmo) {}

float AnalyticalSolver::ballistics(float& out_t, DronePos drone) {
    const float G = 9.81f;
    float           d = currentAmmo.drag;
    float           m = currentAmmo.mass;
    float           l = currentAmmo.lift;
    float attackSpeed = currentDrone.attackSpeed;
    float           z = drone.currentAltitude;
...
float AnalyticalSolver::stop_penalty(DronePos drone) {
    float acc = currentDrone.attackSpeed * currentDrone.attackSpeed / (2.0f * currentDrone.accelerationPath);
...
DropPoint AnalyticalSolver::solve(DronePos drone, Target curTarget) {
    ...
    float h = ballistics(timeToDrop, drone);
    if (h < 0) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        float t_drive = (driveD <= currentDrone.accelerationPath)
            ? std::sqrt(2.0f * driveD / acc)
            : t_acc + (driveD - currentDrone.accelerationPath) / currentDrone.attackSpeed;
