...
namespace dlink {
    ...
    enum PacketType : uint8_t {
        ...
        PKT_AMMO = 0x03, // параметри боєприпасу + hitRadius (один раз настарті)
        PKT_RESULT = 0x04, // вердикт чекера (HIT/MISS) — лише на реальній малиніпо UART назад
    ...
    // type/payload/payloadLen — вихідні (валідні лише при true).
        bool feed(uint8_t byte, uint8_t& outType, uint8_t* outPayload, uint8_t& outLen) {
            //printf("[PARSER] Byte now:0x%02X st now: %d\n", byte, st);
            switch (st) {
        ...
        // перерахувати CRC по TYPE+LEN+payload
                    ...
                    if (crc16(tmp, (size_t)len + 2) == crc_rx) {
                        //printf("[PARSER] OK type=0x%02X len=%d\n", type, len);
                        //fflush(stdout);
                        outType = type; outLen = len;
                        std::memcpy(outPayload, buf, len);
                        return true;
                    }
                    printf("[PARSER] CRC FAIL type=0x%02X len=%d\n", type, len);
                    fflush(stdout);
